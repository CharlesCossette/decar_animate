classdef AnimatedBoxyCar < handle
    ...
    methods
        ...
        function plot(self, r_zw_a, C_ba)
            ...
            self.body.length = (self.l_f + self.l_r)*self.scale;
            self.body.width  = self.l_w*self.scale;
            self.body.height = self.l_h*self.scale;
            ...
            self.wheel_fl.radius = self.wheel_radius*self.scale;
            self.wheel_fl.height = self.wheel_thickness*self.scale;
            self.wheel_fl.faceColor = self.color_wheel;
            r_pz_b = [self.l_f - self.wheel_offset_h/2; self.l_w/2 + self.wheel_offset_v;0]*self.scale;
            r_pz_a = C_ba'*r_pz_b;
            C_pb = getDCMrotVec([pi/2;0;delta]);
            self.wheel_fl.plot(r_pz_a + r_zw_a, C_pb*C_ba);
            % Front right
            self.wheel_fr = AnimatedCylinder();
            self.wheel_fr.radius = self.wheel_radius*self.scale;
            self.wheel_fr.height = self.wheel_thickness*self.scale;
            ...
            self.wheel_rl.radius = self.wheel_radius*self.scale;
            self.wheel_rl.height = self.wheel_thickness*self.scale;
            self.wheel_rl.faceColor = self.color_wheel;
            r_pz_b = [-self.l_r + self.wheel_offset_h; self.l_w/2 + self.wheel_offset_v;0]*self.scale;
            r_pz_a = C_ba'*r_pz_b;
            C_pb = getDCMrotVec([pi/2;0;0]);
            self.wheel_rl.plot(r_pz_a + r_zw_a, C_pb*C_ba);
            % Rear right
            self.wheel_rr = AnimatedCylinder();
            self.wheel_rr.radius = self.wheel_radius*self.scale;
            self.wheel_rr.height = self.wheel_thickness*self.scale;
            ...
            self.axle_fl.radius = self.axle_width*self.scale;
            self.axle_fl.height = self.wheel_offset_v*self.scale;
            self.axle_fl.faceColor = self.color_axle;
            r_pz_b = [self.l_f - self.wheel_offset_h/2; self.l_w/2 + self.wheel_offset_v/2;0]*self.scale;
            r_pz_a = C_ba'*r_pz_b;
            C_pb = getDCMrotVec([pi/2;0;0]);
            self.axle_fl.plot(r_pz_a + r_zw_a, C_pb*C_ba);
            % Front right    
            self.axle_fr = AnimatedCylinder();
            self.axle_fr.radius = self.axle_width*self.scale;
            self.axle_fr.height = self.wheel_offset_v*self.scale;
            ...
            self.axle_rl.radius = self.axle_width*self.scale;
            self.axle_rl.height = self.wheel_offset_v*self.scale;
            self.axle_rl.faceColor = self.color_axle;
            r_pz_b = [-self.l_r + self.wheel_offset_h; self.l_w/2 + self.wheel_offset_v/2;0]*self.scale;
            r_pz_a = C_ba'*r_pz_b;
            C_pb = getDCMrotVec([pi/2;0;0]);
            self.axle_rl.plot(r_pz_a + r_zw_a, C_pb*C_ba);
            % Rear right    
            self.axle_rr = AnimatedCylinder();
            self.axle_rr.radius = self.axle_width*self.scale;
            self.axle_rr.height = self.wheel_offset_v*self.scale;
